...
_GUI_YIELD_CHECKS = 256
_GUI_YIELD_SECONDS = 0.001
...
def _marriage_bridge_expansion(start_id, end_id, individuals, families,
    ...
    while fwd_q:
        ...
        if dist >= half:
            continue
        for nbr, edge in neighbors(curr, individuals, families):
            _check_cancelled(cancel_event)
...
def bfs_find_all_paths(start_id, end_id, individuals, families, top_n=5,
    ...
    if len(found) < top_n:
        bridge_paths = _marriage_bridge_expansion(
            start_id, end_id, individuals, families, max_depth,
    ...
    if len(found) > 1:
        def _label_key(path):
            lbl = _describe_rel(path, individuals,
                                ancestors=_start_ancestors, families=families)
