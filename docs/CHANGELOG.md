...
- **Biological, step, and half-family display** — the parser now preserves GEDCOM parentage metadata (`PEDI`, adoption records, and common `_FREL`/`_MREL` tags) so Profile and graph views can distinguish ordinary, step, half, adopted, and foster relationships. Profiles label non-ordinary relatives while leaving ordinary relatives unqualified, and graph views use alternate line styles plus a compact legend for non-ordinary links, including the biological/default style for comparison.
