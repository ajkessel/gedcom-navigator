"""
Entry point for the GEDCOM Navigator GUI.
...
def main():
    ...
    if sd is None:
        raise RuntimeError(
            "Cannot locate gedcom_navigator_gui.py. "
            "Re-install the package or run from the source tree."
        )
    if sd not in sys.path:
        sys.path.insert(0, sd)
    script = os.path.join(sd, "gedcom_navigator_gui.py")
    spec = importlib.util.spec_from_file_location(
        "gedcom_navigator_gui", script)
    mod = importlib.util.module_from_spec(spec)
    sys.modules["gedcom_navigator_gui"] = mod
