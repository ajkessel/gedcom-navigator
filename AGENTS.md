...
The command-line path has zero third-party runtime dependencies. The graphical path requires the GUI dependencies listed above.
...
```
src/
  gedcom_core.py            # compatibility facade re-exporting parser/search/display helpers
  gedcom_parser.py          # GEDCOM parser + ZIP extraction (no GUI imports)
  ...
  gedcom_gui_results.py     # ResultsMixin: results rendering and navigation
  gedcom_gui_search.py      # SearchMixin: file loading, filtering, search launching
  gedcom_graph_export.py    # SVG/PNG export helpers for relationship graph canvases
  gedcom_name_search.py     # shared ID, token, and fuzzy name matching
  gedcom_platform.py        # process-level platform integration hooks
  ...
  gedcom_tooltip.py         # Shared tooltip helper
...
1. **Parsing** (`gedcom_parser.build_model`, re-exported by `gedcom_core`): Two-pass GEDCOM parser. Pass 1 collects `_MTTAG` tag definitions; Pass 2 parses `INDI` and `FAM` records. Returns `individuals`, `families`, `tag_records` dicts plus optional warning/error details.
...
3. **BFS search** (`gedcom_search`, re-exported by `gedcom_core`):
